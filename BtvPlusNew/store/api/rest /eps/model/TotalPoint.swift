...
struct TotalPointInfo : Decodable {
    ...
    private(set) var tvpoint:TvPoint?    // TV포인트 정보 ※ API 응답 코드가 '0000'(OK)인 경우에만 응답
...
struct TotalPoint : Decodable {
    ...
    private(set) var useBpointRun:Int?   // "월정액 B포인트 자동 차감 여부 - true: 자동 차감 사용 - false: 자동 차감 미사용 ※ API 응답 코드가 '0000'(OK)인 경우에만 응답"
    private(set) var usableBpoints:Int?  // "사용가능 B포인트 총액 ※ API 응답 코드가 '0000'(OK)인 경우에만 응답"
    private(set) var expireBpoints:Int?  // "만료예정 B포인트 총액 ※ API 응답 코드가 '0000'(OK)인 경우에만 응답"
    private(set) var bpoints:[BPointItem]?    // B포인트 정보 ※ API 응답 코드가 '0000'(OK)인 경우에만 응답
    private(set) var tmembership:TMembership?   // T멤버십 정보 ※ API 응답 코드가 '0000'(OK)인 경우에만 응답 등록된 T멤버십카드가 없을 경우 null 또는 빈값 응답
    private(set) var ocbMasterSequence:Int?  // 대표 OK캐쉬백 카드 순번 ※ API 응답 코드가 '0000'(OK)인 경우에만 응답
    private(set) var ocbList:[OcbItem]?    // B포인트 정보 ※ API 응답 코드가 '0000'(OK)인 경우에만 응답
    private(set) var tvpoint:TvPoint?    // TV포인트 정보 ※ API 응답 코드가 '0000'(OK)인 경우에만 응답
    private(set) var tvpay:TvPay?    // TV페이 정보 ※ API 응답 코드가 '0000'(OK)인 경우에만 응답
}
